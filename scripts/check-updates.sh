...
check_packages_for_updates() {
	...
	for package_name in $available_packages; do
		...
		if [ "$translated_package_name" != "/hidden/" ]; then
			local api_response
			api_response=$(curl --silent --location "https://repology.org/api/v1/project/$translated_package_name")
			package_latest_version=$(echo "$api_response" | jq -r ".[] | select(.visiblename == \"$translated_package_name\").version" | sort -rV | head -n1)

			#if [ -z "$package_latest_version" ]; then
			#	package_latest_version=$(echo "$api_response" | jq -r ".[] | select(.visiblename == \"$package_name\").version" | sort -rV | head -n1)
			#fi
		else
...
echo
echo "Checking packages for updates. Please wait."
echo
