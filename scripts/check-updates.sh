...
translate_package_name() {
	...
	case "$1" in
		...
		# Custom package.
		fltk-editor) translated_name="/hidden/";;

		geth) translated_name="go:ethereum";;

		git-subtree) translated_name="git";;
		...
		# Custom package.
		libposix-shm) translated_name="/hidden/";;

		libcairo-x) translated_name="cairo";;

		libfm-extra) translated_name="libfm";;
		...
		xorg-mkfontdir) translated_name="mkfontdir";;

		xorg-mkfontscale) translated_name="mkfontscale";;

		xorg-server-xvfb) translated_name="xorg-server";;

		xorg-twm) translated_name="twm";;

		xorg-xauth) translated_name="xauth";;

		xorg-xcalc) translated_name="xcalc";;

		xorg-xclock) translated_name="xclock";;

		xorg-xdpyinfo) translated_name="xdpyinfo";;

		xorg-xev) translated_name="xev";;

		xorg-xmessage) translated_name="xmessage";;

		xorg-xsetroot) translated_name="xsetroot";;

		mpv-x) translated_name="mpv";;

		pango-x) translated_name="pango";;

		pinentry-gtk) translated_name="pinentry";;

		poppler-glib) translated_name="poppler";;

		python-six|python2-six) translated_name="python:six";;

		python-xlib|python2-xlib) translated_name="python:xlib";;

		qemu-common|qemu-system-*|qemu-user-*) translated_name="qemu";;

		qt5-base) translated_name="qt5";;

		the-powder-toy) translated_name="powder-toy";;

		transmission-gtk) translated_name="transmission";;

		ttf-dejavu) translated_name="fonts:dejavu";;

		unicorn) translated_name="unicorn-emulator";;

		wireshark-*|tshark) translated_name="wireshark";;

		# Custom package.
		xterm) translated_name="/hidden/";;

		xorg-font-util) translated_name="xorg-font-utils";;
...
check_packages_for_updates() {
	...
	for package_name in $available_packages; do
		...
		local repo
		for repo in arch aur alpine_edge linuxbrew debian_unstable freebsd openbsd; do
