...
termux_step_host_build() {
	...
	# Cross-compiling elisp from 64 bit guile to 32 bit for i686 and arm doesn't
	# work properly so we are using a precompiled boot.go.
	# This is not an issue because its reproducible. All 32 bit boot.go are bit
	# identitical.
	# To actually derive 32 boot.go ourselves requires 32bit host-build guile
	# and to get that to work you also need to patch 32 bit libgc so it works
	# under docker (until libgc 7.7 is released anyway). This is really annoying
	# and unnecessary.
